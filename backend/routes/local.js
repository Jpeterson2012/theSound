...
router.get('/', async (req, res) => {
  sql = 'CREATE TABLE IF NOT EXISTS local (id INT AUTO_INCREMENT PRIMARY KEY, title VARCHAR(50), artist VARCHAR(60), album VARCHAR(100), track_number VARCHAR(10), image MEDIUMTEXT)'
  ...
  con.query(sql, function(err, result) {
      ...
      if (empty == 1) {
        sql = 'SELECT title, artist, album, track_number, image from local'
        con.query(sql, function (err, result) {
          if (err) throw err;
          var records = {}
          var items = []
        ...
        let apiRequestLoop = function(){
          let promiseArray = []
          files.map(a => promiseArray.push(get_song(a).then(tag => { return {id: b++, title: tag?.tags?.title, artist: tag?.tags?.artist, album: tag?.tags?.album, track_number: tag?.tags?.track, image: tag?.tags?.picture}}))
          )
        ...
        apiRequestLoop().then(a => a.map((b,i,arr) => {
          var values = []
          values.push([b.title, b.artist, b.album, b.track_number, JSON.stringify(b.picture)])
          var sql = "INSERT INTO local (title, artist, album, track_number, image) VALUES ?"
