...
router.get('/:id', async (req, res) => {
    ...
    while(true) {
        ...
        data.items?.map(a => a ? arr.push({album_id: a.track?.album.id, images: (a.track?.album === null ? album : a.track?.album.images), name: a.track?.name, duration_ms: a.track?.duration_ms, artists: a.track?.artists, uri: a.track?.uri}) : null)
        a.items = arr
        a.total = data.total
