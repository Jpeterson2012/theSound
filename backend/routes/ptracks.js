...
router.get('/:id', async (req, res) => {
    ...
    while(true) {
        ...
        data.items?.map(a => a ? arr.push({album: (a.track?.album === null ? album : a.track?.album.images), name: a.track?.name, duration_ms: a.track?.duration_ms, artists: a.track?.artists, uri: a.track?.uri}) : null)
        a.items = arr
        a.total = data.total
