...
async function makePlaylists() {
  ...
  try{
    for(let i = 0; i < vars.length; i++) {
      ...
      while (pages < 1000) {
        let url = `https://api.spotify.com/v1/search?q=genre:"${vars[i]}"&type=track&offset=${pages}&limit=50`;
        ...
        data.tracks.items.map(a => a && temp2.push({...a, date_added: new Date(a.added_at ?? Date.now())}));
