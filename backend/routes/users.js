...
router.get('/', async function(req, res, next) {  
  ...
  //             var pages = 0
  //             var pName = 1
  //             let sql = `CREATE TABLE IF NOT EXISTS ${vars[i].replace(' ','').replace('&','and').replace('-','')} (id INT AUTO_INCREMENT PRIMARY KEY, playlist_id VARCHAR(70), images LONGTEXT, name VARCHAR(100), public BOOL, uri varCHAR(40), tracks JSON)`
  ...
  //               sql = `INSERT INTO ${vars[i].replace(' ','').replace('&','and').replace('-','')} (playlist_id, images, name, public, uri, tracks) VALUES ?`
